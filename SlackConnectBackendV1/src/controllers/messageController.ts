    ...
    try {
      ...
      try {
        ...
        debug.tests.bot_token_test = {
          ...
          sample_channels: botChannels.slice(0, 3).map((c: any) => ({ id: c.id, name: c.name }))
        };
      } catch (error) {
        debug.tests.bot_token_test = {
          success: false,
      ...
      try {
        ...
        debug.tests.user_token_test = {
          ...
          sample_channels: userChannels.slice(0, 3).map((c: any) => ({ id: c.id, name: c.name }))
        };
      } catch (error) {
        debug.tests.user_token_test = {
          success: false,
  ...
  public getChannels = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      try {
        ...
        console.log('Bot token approach failed, trying user token approach...');
        ...
        try {
          ...
          console.log('User token approach also failed, using fallback...');
